...
def _raw_coords_features(X, E_idx):
    B, L, _, _ = X.shape

    # Build local frame from N, CA, C — same frame as _orientations_coarse_gl_tuple
    ...
    # n_0 and b_1 are [B, L-1, 3] after subsampling — pad to [B, L, 3]
    n_0 = F.pad(n_0, (0, 0, 0, 1), 'constant', 0)   # [B, L, 3]
    b_1 = F.pad(b_1, (0, 0, 0, 1), 'constant', 0)   # [B, L, 3]

    Q = torch.stack((b_1, n_0, torch.cross(b_1, n_0, dim=-1)), dim=-1)  # [B, L, 3, 3]
    Q_exp = Q.unsqueeze(2)                                               # [B, L, 1, 3, 3]                                         # [B, L, 1, 3, 3]

    # Node features: 4 atoms of residue i in its own frame → [B, L, 12]
    dX_node = X - CA.unsqueeze(2)                                        # [B, L, 4, 3]
    dU_node = torch.matmul(
        Q_exp.transpose(-1,-2),
        dX_node.unsqueeze(-1)
    ).squeeze(-1)                                                        # [B, L, 4, 3]
    V_raw = dU_node.reshape(B, L, 12)

    # Edge features: 4 atoms of neighbour j in frame of residue i → [B, L, K, 12]
    N_nb  = gather_nodes(X[:,:,0,:], E_idx)
    CA_nb = gather_nodes(X[:,:,1,:], E_idx)
    C_nb  = gather_nodes(X[:,:,2,:], E_idx)
    O_nb  = gather_nodes(X[:,:,3,:], E_idx)

    atoms_nb = torch.stack([N_nb, CA_nb, C_nb, O_nb], dim=3)            # [B, L, K, 4, 3]
    dX_edge  = atoms_nb - CA.unsqueeze(2).unsqueeze(3)                   # [B, L, K, 4, 3]
    dU_edge  = torch.matmul(
        Q_exp.unsqueeze(3).transpose(-1,-2),                             # [B, L, 1, 1, 3, 3]
        dX_edge.unsqueeze(-1)                                            # [B, L, K, 4, 3, 1]
    ).squeeze(-1)                                                        # [B, L, K, 4, 3]
    E_raw = dU_edge.reshape(B, L, E_idx.shape[2], 12)
